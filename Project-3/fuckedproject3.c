    ...
    */
    ...
    for (i = 0; i< my_arg->total_no_book; i++){  // loop until goal of book numbers is reached
        ...
        */
        ...
        //printf(" %s %d\n", my_arg->pblshr_strct->buffer[index], index);
        // int k;
        // for (k = 0; k < my_arg->pblshr_strct->buffer_size; k++){
        //     printf(" %s \n", my_arg->pblshr_strct->buffer[k]);
        // }
    ...
    */
    ...
    while (1){
        ...
        if (my_arg->publisher_list == NULL){
            ...
            int i, is_full=1;
            for (i=0; i<sizeof(package)/sizeof(char*);i++){
                if (*package[i] == '\0'){
                    is_full = 0;
                    break;
                }
            ...
            if (is_full){
                ...
                int j;
                for (j=0; j<sizeof(package)/sizeof(char*);j++){
                    printf("%s, ",package[j]);
                    package[j] = '\0';
                }
                printf(".\n");
                ...
                int j; // 0 1 2 3 4 5
                for (j = 0; j < my_arg->publisher_list[random_buffer_index]->buffer_size; j++){
                    pubBuff[j] = pubBuff[j + 1];
                }
                pubBuff[j] = '\0';
